function [alpha_var, xmin_var, nn]=plvar(x, varargin)
...
switch f_dattype,
    ...
    case 'REAL',
        ...
        for B=1:size(bof,1)
            ...
            z     = y(y>=ymin);
            nn     = length(z); 
            alpha_var = 1 + nn ./ sum( log(z./ymin) );
            % store distribution of estimated parameter values
            bof(B,:) = [nn ymin alpha_var];
            if ~quiet && B>1,
            ...
            for xm=1:length(ymins)
                ...
                if fcatch==0
                    try
                        ...
                        zdiff = sum( repmat((1:xmin_var-1)',1,length(vec)).^-repmat(vec,xmin_var-1,1) ,1);
                        L = -vec.*sum(log(z)) - nn.*log(zvec - zdiff);
                    catch
                ...
                if fcatch==1
                    ...
                    L       = -Inf*ones(size(vec));
                    slogz   = sum(log(z));
                    xminvec = (1:xmin_var-1);
                    for k=1:length(vec)
                        L(k) = -vec(k)*slogz - nn*log(zvec(k) - sum(xminvec.^-vec(k)));
                    end;
                end;
                [Y,I] = max(L);
                % compute KS statistic
                fit = cumsum((((xmin_var:ymax).^-vec(I)))./ (zvec(I) - sum((1:xmin_var-1).^-vec(I))));
                cdi = cumsum(hist(z,xmin_var:ymax)./nn);
